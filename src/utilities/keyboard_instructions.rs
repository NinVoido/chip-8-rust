...
impl crate::utilities::cpu::Cpu{
    ...
    fn skip_if_pressed(mut self, x: u8, input: &WinitInputHelper){
    ...
    fn skip_if_not_pressed(mut self, x: u8, input: &WinitInputHelper){
       if !input.key_held(self::to_keycode(self.registers[x as usize])){
         self.pc += 2
        }
    }
    ...
    fn scan_key(mut self, x: u8, input: &WinitInputHelper){
